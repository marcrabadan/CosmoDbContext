using CosmosDbFramework.Internal.Attributes;
using System;
...
namespace CosmosDbFramework.Internal
{
    public sealed class PropertyDiscovery<TFrom> where TFrom : class
    {
        ...
        {
            var setterMethod = typeof(TFrom).GetMethods().Single(m => m.Name == implementedMethodName && m.IsGenericMethodDefinition);
